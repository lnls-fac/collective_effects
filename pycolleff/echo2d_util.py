...
def circle_out(x_c,y_c,R,theta):
    t = EchoObj()
...
def circle_inn(x_c,y_c,R,theta):
    t = EchoObj()
...
def reflect(t_in):
    if isinstance(t_out,list) and isinstance(t_out[0],EchoObj):
...
def translate(t_in,delta):
    if isinstance(t_out,list) and isinstance(t_out[0],EchoObj):
        for i in range(len(t_out)):
            t_out[-i-1] = translate(t_in[i],delta)
    else:
        t_out = t_in.copy()
        t_out[[0,2]] += delta
...
def create_linear_taper(fname=None, r_in=0.012, r_out=0.004, t = 20,
    ...
    p3 = _np.array([t_in*(r_in-r_out)         , r_out])
    ...
    if C is None:
        ...
        if r_in > r_out:
            ...
            t3 = circle_inn(p3[0],p3[1],r_inn,theta)
        else:
            t1 = circle_inn(p2[0],p2[1],r_out,theta)
            t3 = circle_out(p3[0],p3[1],r_inn,theta)
...
def create_collimator(fname,R_in=0.012,R_out=None, r=0.004,
